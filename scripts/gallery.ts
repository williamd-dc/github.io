"use strict";
      ...
      document.addEventListener("routeLoaded", () => {
        if(location.hash.slice(1) != "/gallery") return;
        ...
        function showSlides(n) {
          ...
          var dots = document.getElementsByClassName("demo") as HTMLCollectionOf<HTMLImageElement>;
          ...
          for (i = 0; i < slides.length; i++) {
            (slides[i] as ElementCSSInlineStyle).style.display = "none";
          }
          for (i = 0; i < dots.length; i++) {
            (dots[i] as HTMLElement).className = dots[i].className.replace(" active", "");
          }
          (slides[slideIndex - 1] as ElementCSSInlineStyle).style.display = "block";
          (dots[slideIndex - 1] as HTMLElement).className += " active";
          captionText.innerHTML = (dots[slideIndex - 1] as HTMLImageElement).alt;
        }
      });